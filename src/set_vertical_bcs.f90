...
subroutine set_vertical_bcs(LogRho,LogNS,Vel_GD,Temp, LogINS, iVel, VertVel)
  ...
  real :: interp_ktemp_space,interp_zonal_space,interp_merid_space,&
          ...
          interp_n_space,rho,interp_vert_space
...
!!! Need a 5-point Stencil
  ...
  real :: MeshCoefm0, MeshCoefm1, &
          ...
          MeshCoefm4
  real,dimension(-1:0) :: waccm_zonal,waccm_merid,waccm_ktemp, waccm_vert
...
!        endif 
    ...
    do iSpecies = 1, nSpecies
         ...
         VertVel(iAlt  ,iSpecies) = VertVel(iAlt+1,iSpecies) - &
                                     dAlt_F(iAlt+1)*dVertVel
         !if (LowBVertVel) then
         !   VertVel(iAlt  ,iSpecies) = waccm_vert(iAlt)
         !endif
         !if (UseLowerBC) VertVel(iAlt,iSpecies) = 0.0 ! Garima
    enddo ! nSpecies
  ...
  if (UseLowerBC) then
      do iAlt=-1,0
        Alt=Altitude_G(iAlt)/1000.0
        call interp_3Dspace(Alt,Lat,Lon,interp_ktemp_space,interp_zonal_space,&
            interp_merid_space,interp_density_space,interp_o_space,interp_o2_space,&
            interp_nitrogen_space,interp_co2_space,interp_no_space,&
            interp_n_space,interp_vert_space)
        
        if (LowBMSISonlyO) then
            LogNS(iAlt,2:3) = (/log(interp_o2_space),&
                                log(interp_nitrogen_space)/)
            NS(iAlt,2:3)=(/interp_o2_space,&
                            interp_nitrogen_space/)
        else
            LogNS(iAlt,1:3) = (/log(interp_o_space),log(interp_o2_space),&
                                log(interp_nitrogen_space)/)
            NS(iAlt,1:3)=(/interp_o_space,interp_o2_space,&
                                interp_nitrogen_space/)
        endif
	!,log(interp_n_space),&
        !LogNS(iAlt,4)=log(interp_n_space)
            
	LogNS(iAlt,5)=log(interp_no_space) !CO2 ?
        !NS(iAlt,4)=interp_n_space
	!,interp_n_space,&
        NS(iAlt,5)= interp_no_space
        ...
        if (LowBMSISonlyO) then
            LogNS(iAlt,1)=LogNS_Species(1)
            NS(iAlt,1)=exp(LogNS_Species(1))
        endif

	LogNS(iAlt,4)=LogNS_Species(4)
	NS(iAlt,4)=exp(LogNS_Species(4))

        LogNS(iAlt,6)=LogNS_Species(6)
        NS(iAlt,6)=exp(LogNS_Species(6))

        !rho=(Mass(1)*interp_o_space)+(Mass(2)*interp_o2_space)+(Mass(3)*interp_nitrogen_space)+&
        !    (Mass(4)*NS(iAlt,4))+(Mass(5)*interp_no_space)+(Mass(6)*NS(iAlt,6))
        rho=(Mass(1)*NS(iAlt,1))+(Mass(2)*NS(iAlt,2))+(Mass(3)*NS(iAlt,3))+&
            (Mass(4)*NS(iAlt,4))+(Mass(5)*NS(iAlt,5))+(Mass(6)*NS(iAlt,6))
        LogRho(iAlt)=log(rho)
        if (.not. DuringPerturb) temp(iAlt) = interp_ktemp_space
        vel_gd(iAlt,iEast_) = interp_zonal_space
        vel_gd(iAlt,iNorth_) = interp_merid_space
        Vel_GD(iAlt,iUp_)=interp_vert_space
        temp(iAlt)=interp_ktemp_space
        !waccm_zonal(iAlt)=interp_zonal_space
        !waccm_merid(iAlt)=interp_merid_space
        !waccm_ktemp(iAlt)=interp_ktemp_space
        !waccm_vert(iAlt)=interp_vert_space

          !do iAlt=-1,0
          !  if (UseLowerBC) then
          !      vel_gd(iAlt,iEast_) = waccm_zonal(iAlt)
          !      vel_gd(iAlt,iNorth_) = waccm_merid(iAlt)
          !      temp(iAlt) = waccm_ktemp(iAlt)
          !  endif
          !enddo
      enddo
  endif
